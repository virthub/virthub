...
void vres_page_lock_init()
{
	...
	for (i = 0; i < VRES_PAGE_LOCK_GROUP_SIZE; i++) {
		...
		vres_page_lock_group[i].head = rbtree_create();
	}
...
static inline int vres_page_lock_compare(char *l1, char *l2)
{
	vres_page_lock_desc_t *desc1 = (vres_page_lock_desc_t *)l1;
	vres_page_lock_desc_t *desc2 = (vres_page_lock_desc_t *)l2;

	return memcmp(desc1->entry, desc2->entry, sizeof(((vres_page_lock_desc_t *)0)->entry));
}
...
{
	return rbtree_lookup(group->head, (void *)desc, vres_page_lock_compare);
}


static inline void vres_page_lock_insert(vres_page_lock_group_t *group, vres_page_lock_t *lock)
{
	rbtree_insert(group->head, (void *)&lock->desc, (void *)lock, vres_page_lock_compare);
}
...
{
	rbtree_delete(group->head, (void *)&lock->desc, vres_page_lock_compare);
...
void *vres_page_get(vres_t *resource, vres_page_t **page, int flags)
{
	...
	entry = tmp_get_entry(path, sizeof(vres_page_t), flags & (VRES_RDWR | VRES_RDONLY | VRES_CREAT));
	if (!entry) {
		vres_page_unlock(resource);
		return NULL;
	}
	*page = tmp_get_desc(entry, vres_page_t);
...
static int vres_page_update(vres_page_t *page, char *buf)
{
	int i, j;
	int ret = 0;
	int dirty = 0;
...
int vres_page_search_holder_list(vres_t *resource, vres_page_t *page, vres_id_t id)
{
	...
	if (nr_silent_holders > 0) {
		...
		if (entry) {
			...
			holders = tmp_get_desc(entry, vres_id_t);
...
int vres_page_update_holder_list(vres_t *resource, vres_page_t *page, vres_id_t *holders, int nr_holders)
{
	...
	if (nr_silent_holders > 0) {
		...
		if (tmp_size(filp) / sizeof(vres_id_t) != page->nr_silent_holders) {
			vres_log_err(resource, "invalid file length");
			ret = -EINVAL;
			goto out;
		}

		tmp_seek(filp, 0, SEEK_END);
		if (tmp_write(&holders[nr_holders], sizeof(vres_id_t), nr_silent_holders, filp) != nr_silent_holders) {
			vres_log_err(resource, "failed to update holder list");
			ret = -EIO;
		}
out:
		tmp_close(filp);
...
static int vres_page_wrprotect(vres_t *resource, vres_page_t *page)
{
	int ret;
	char *buf;
	...
	ret = sys_shm_wrprotect(resource->key, vres_get_off(resource), buf);
	if (ret) {
		if (ENOENT == errno) {
			ret = 0;
			goto protect;
	...
	if (ret) {
		vres_log_err(resource, "failed to update");
		goto out;
	}
protect:
...
static int vres_page_rdprotect(vres_t *resource, vres_page_t *page)
{
	int ret = 0;
	char *buf = NULL;
	...
	ret = sys_shm_rdprotect(resource->key, vres_get_off(resource), buf);
	if (ret) {
		if (ENOENT == errno) {
			ret = 0;
			goto protect;
		} else {
			vres_log_err(resource, "failed to protect (%d)", errno);
			goto out;
		}
	}
...
int vres_page_protect(vres_t *resource, vres_page_t *page)
{
	...
	if (flags & VRES_RDONLY)
		return vres_page_wrprotect(resource, page);
	else if (flags & VRES_RDWR)
		return vres_page_rdprotect(resource, page);
