...
int vres_shm_check(vres_t *resource, struct shmid_ds *shp)
{
	...
	if (tmp_read(shp, sizeof(struct shmid_ds), 1, filp) != 1)
		ret = -EIO;
...
int vres_shm_is_owner(vres_t *resource)
{
#if defined(STATIC_MANAGER)
...
int vres_shm_init(vres_t *resource)
{
	...
	if (!vres_is_owner(resource)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to sync time");
			return ret;
		}
		off = vres_get_time_offset(start, time);
	}
	ret = vres_prio_create(resource, off);
	if (ret) {
		vres_log_err(resource, "failed to create prio");
		return ret;
	}
#endif
	return ret;
...
int vres_shm_create(vres_t *resource)
{
	...
	if (!filp) {
		vres_log_err(resource, "failed to create shm");
		return -ENOENT;
	}
...
int vres_shm_save_req(vres_page_t *page, vres_req_t *req)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to save request");
		return ret;
	}
	vres_pg_mkredo(page);
...
int vres_shm_update(vres_t *resource, struct shmid_ds *shp)
{
	...
	if (tmp_write(shp, sizeof(struct shmid_ds), 1, filp) != 1)
		ret = -EIO;
...
static inline int vres_shm_get_peer_info(vres_t *resource, vres_page_t *page, vres_shm_peer_info_t *info)
{
	...
	if (flags & VRES_RDWR) {
		...
		for (i = 0; i < page->nr_holders; i++) {
			int ret = vres_get_peer(page->holders[i], &info->list[i]);
	...
	return vres_get_peer(page->owner, &info->owner);
}
...
static inline int vres_shm_cache_add(vres_page_t *page, vres_req_t *req)
{
	...
	if (entry) {
		int total = tmp_items_count(entry, sizeof(vres_shm_cache_t));

		if (total > 0) {
			int i;
			vres_shm_cache_t *caches = tmp_get_desc(entry, vres_shm_cache_t);
	...
	if (!updated) {
		...
		cache.id = id;
		ret = tmp_append(entry, &cache, sizeof(vres_shm_cache_t));
	}
...
static inline int vres_shm_cache_check(vres_page_t *page, vres_req_t *req)
{
	int i;
	int total;
	int ret = 0;
	...
	vres_get_cache_path(resource, path);
	entry = tmp_get_entry(path, 0, TMP_RDONLY);
	if (!entry)
		return 0;

	caches = tmp_get_desc(entry, vres_shm_cache_t);
	total = tmp_items_count(entry, sizeof(vres_shm_cache_t));
	for (i = 0; i < total; i++) {
...
int vres_shm_fast_reply(vres_page_t *page, vres_req_t *req)
{
	...
	if (vres_pg_active(page) && (hid > 0)) {
		...
		if (vres_page_get_diff(page, arg->version, diff)) {
			vres_log_err(resource, "failed to get diff info");
			return -EINVAL;
		}

		if (vres_shm_calc_htab(htab, VRES_LINE_MAX, nr_peers)) {
			vres_log_err(resource, "failed to calculate htab");
			return -EINVAL;
		}
	...
	if ((nr_lines > 0) || reply || head || tail) {
		...
		if (tail) {
			...
			if (flags & VRES_RDONLY)
				size += page->nr_holders * sizeof(vres_desc_t);
		}
		...
		if (tail) {
			...
			if (ret) {
				vres_log_err(resource, "failed to get peer info");
				goto out;
			}
			new_arg->arg.owner = page->owner;
			vres_set_flags(&res, VRES_PEER);
		}
...
cache:
	if (vres_pg_own(page) || (hid > 0) || notify) {
		if (vres_shm_cache_add(page, req)) {
			vres_log_err(resource, "failed to cache");
			ret = -EFAULT;
		}
	}
...
int vres_shm_request_holders(vres_page_t *page, vres_req_t *req)
{
	...
	if (req->length > VRES_IO_MAX) {
		vres_log_err(resource, "invalid request length");
		return -EINVAL;
	}

	// The clk piggybacked on req has to be updated.
	arg->clk = page->clk;
	arg->owner = resource->owner;
	arg->cmd = VRES_SHM_NOTIFY_HOLDER;

#if defined(STATIC_MANAGER)
	ret = vres_smgr_request_holders(page, req);
#elif defined(DYNAMIC_MANAGER)
	ret = vres_dmgr_request_holders(page, req);
#else
...
int vres_shm_update_holder(vres_page_t *page, vres_req_t *req)
{
	...
	if (flags & VRES_RDONLY) {
		...
		if (ret) {
			vres_log_err(resource, "failed to add holder");
			return ret;
		}
	...
	ret = vres_page_protect(resource, page);
	if (ret) {
		vres_log_err(resource, "failed to protect");
		return ret;
	}
...
#if defined(DYNAMIC_OWNER)
	else if (!vres_pg_own(page)) {
		vres_shmfault_arg_t *arg = (vres_shmfault_arg_t *)req->buf;
...
int vres_shm_request_silent_holders(vres_page_t *page, vres_req_t *req)
{
	...
	if ((flags & VRES_RDWR) && !vres_shm_is_silent_holder(page) && vres_pg_active(page)) {
		if (page->nr_silent_holders > 0) {
			...
			silent_holders = tmp_get_desc(entry, vres_id_t);
...
int vres_shm_do_check_coverage(vres_page_t *page, int line)
{
...
int vres_shm_check_coverage(vres_t *resource, vres_page_t *page, int line)
{
#if defined(STATIC_MANAGER)
	return vres_smgr_check_coverage(resource, page, line);
#elif defined(DYNAMIC_MANAGER)
	return vres_dmgr_check_coverage(resource, page, line);
#else
	return vres_shm_do_check_coverage(page, line);
#endif
}


void vres_shm_do_check_reply(vres_t *resource, vres_page_t *page, int total, int *head, int *tail, int *reply)
{
	int flags = vres_get_flags(resource);
	vres_id_t src = vres_get_id(resource);
...
void vres_shm_check_reply(vres_t *resource, vres_page_t *page, int total, int *head, int *tail, int *reply)
{
#if defined(STATIC_MANAGER)
	vres_smgr_check_reply(resource, page, total, head, tail, reply);
#elif defined(DYNAMIC_MANAGER)
	vres_dmgr_check_reply(resource, page, total, head, tail, reply);
#else
...
int vres_shm_do_check_holder(vres_page_t *page, vres_req_t *req)
{
	...
	for (i = 0; i < VRES_LINE_MAX; i++) {
		if (diff[i] != 0) {
			total++;
			if (vres_shm_check_coverage(resource, page, i)) {
				nr_lines++;
				if (1 == total)
					head = 1;
			}
	...
	if ((nr_lines > 0) || reply || head || tail) {
		...
		if (nr_lines > 0) {
			...
			for (i = 0; i < VRES_LINE_MAX; i++) {
				if (diff[i] & vres_shm_check_coverage(resource, page, i)) {
					vres_line_t *line = &new_arg->lines[j];
...
#ifdef CHECK_PRIORITY
static inline int vres_shm_need_priority(vres_t *resource, vres_page_t *page)
{
	int flags = vres_get_flags(resource);

	if (vres_pg_own(page) && !(flags & VRES_OWN) && (vres_pg_write(page) || (flags & VRES_RDWR)))
		return 1;
	else
		return 0;
}
...
int vres_shm_do_check_owner(vres_page_t *page, vres_req_t *req)
{
	...
	if (vres_shm_need_priority(resource, page)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to update priority");
			return ret;
		}
		need_priority = 1;
	}
	...
	if (vres_pg_own(page)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to update holder");
			return ret;
		}
	} else {
		ret = vres_shm_update_owner(page, req);
		if (ret) {
			vres_log_err(resource, "failed to update owner");
			return ret;
		}
	}
...
int vres_shm_save_updates(vres_t *resource, vres_page_t *page, vres_line_t *lines, int nr_lines)
{
	...
	if (!lines || (nr_lines <= 0)) {
		vres_log_err(resource, "no content");
		return -EINVAL;
	}
	...
	ret = tmp_append(entry, lines, sizeof(vres_line_t) * nr_lines);
	tmp_put_entry(entry);
	if (ret) {
		vres_log_err(resource, "failed");
		return -EFAULT;
	}
	...
	log_shm_save_updates(resource, nr_lines);
	return ret;
}
...
void vres_shm_load_updates(vres_t *resource, vres_page_t *page)
{
	...
	lines = tmp_get_desc(entry, vres_line_t);
	total = tmp_items_count(entry, sizeof(vres_line_t));
	for (i = 0; i < total; i++) {
...
int vres_shm_do_change_owner(vres_page_t *page, vres_req_t *req)
{
	...
	if (ret) {
		vres_log_err(resource, "invalid member");
		return ret;
	}
	...
	if (update) {
		...
		if (ret) {
			vres_log_err(resource, "failed to update member");
			return ret;
		}
...
int vres_shm_check_holder(vres_req_t *req)
{
	...
	ret = vres_shm_request_silent_holders(page, req);
	if (ret) {
		vres_log_err(resource, "failed to send to silent holders");
		goto out;
	}

	ret = vres_shm_do_check_holder(page, req);
	if (ret) {
		vres_log_err(resource, "failed to check holder");
		goto out;
	}
...
#ifdef FAST_REPLY
	if (!vres_shm_cache_check(page, req))
		ret = vres_shm_fast_reply(page, req);
	if (-EAGAIN == ret)
#endif
		ret = vres_shm_do_check_holder(page, req);
...
int vres_shm_wakeup(vres_t *resource, vres_page_t *page)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to set event");
		return ret;
	}
...
int vres_shm_deliver(vres_page_t *page, vres_req_t *req)
{
	...
	if (src != resource->owner) {
		vres_log_err(resource, "invalid src, src=%d", src);
		return -EINVAL;
	}
	...
	if (ret) {
		vres_log_err(resource, "failed to wake up");
		return ret;
	}
...
int vres_shm_update_peers(vres_t *resource, vres_page_t *page, vres_shm_peer_info_t *info)
{
	...
	if (flags & VRES_RDONLY) {
		...
		if ((info->total > VRES_PAGE_NR_HOLDERS) || (info->total < 0)) {
			vres_log_err(resource, "invalid peer info");
			return -EINVAL;
		}

		for (i = 0; i < info->total; i++) {
			ret = vres_page_add_holder(resource, page, info->list[i].id);
			if (ret) {
				vres_log_err(resource, "failed to add holder");
				return ret;
			}

			ret = vres_save_peer(&info->list[i]);
			if (ret) {
				vres_log_err(resource, "failed to save addr");
				return ret;
			}
		}
	}
	page->owner = info->owner.id;
	ret = vres_save_peer(&info->owner);
	if (ret) {
		vres_log_err(resource, "failed to save addr");
		return ret;
	}
...
int vres_shm_notify_proposer(vres_req_t *req)
{
	...
	if (flags & VRES_PEER) {
		...
		if (ret) {
			vres_log_err(resource, "failed to update peers");
			goto out;
		}
	...
	if (nr_lines > 0) {
		...
		if (total < 0) {
			vres_log_err(resource, "failed to save updates");
			ret = -EINVAL;
			goto out;
		}
	}
	...
	if (flags & VRES_RDWR) {
		...
		if (total == arg->total) {
			if (vres_pg_ready(page))
				...
				vres_pg_mkuptodate(page);
		}
	} else {
		vres_log_err(resource, "invalid flag");
		ret = -EINVAL;
		goto out;
	}

	if (ready) {
		log_shm_deliver_event(req);
...
int vres_shm_call(vres_arg_t *arg)
{
	...
	tmp_entry_t *entry = (tmp_entry_t *)arg->entry;
#ifdef CHECK_PRIORITY
	int need_priority = 0;
	vres_page_t *page = tmp_get_desc(entry, vres_page_t);
...
#endif
	ret = klnk_call_broadcast(arg);
	if (ret) {
		vres_log_err(resource, "failed to broadcast");
		goto out;
	}
#ifdef CHECK_PRIORITY
	if (need_priority) {
...
int vres_shm_handle_zeropage(vres_t *resource, vres_page_t *page)
{
	...
	if (resource->owner == src) {
		...
		if (vres_get_peer(page->owner, &info->owner)) {
			vres_log_err(resource, "failed to get peer");
			ret = -EFAULT;
			goto out;
		}
...
static inline int vres_shm_request_owner(vres_page_t *page, vres_req_t *req, vres_id_t *dest)
{
	...
	ret = klnk_io_sync(&req->resource, req->buf, req->length, NULL, 0, dest);
	arg->cmd = cmd;
	return ret;
}
...
static inline int vres_shm_can_forward()
{
#if defined(DYNAMIC_OWNER) || defined(DYNAMIC_MANAGER)
	return 1;
#else
	return 0;
#endif
...
static inline int vres_shm_need_forward(vres_page_t *page, vres_req_t *req)
{
	if (vres_shm_can_forward()) {
		vres_shmfault_arg_t *arg = (vres_shmfault_arg_t *)req->buf;
		vres_peers_t *peers = &arg->peers;
		vres_t *resource = &req->resource;

		return !vres_pg_own(page) && (peers->list[0] == resource->owner);
	} else
		return 0;
}
...
static inline int vres_shm_check_priority(vres_page_t *page, vres_req_t *req, int flags)
{
	...
	if (vres_shm_need_priority(resource, page) && !(flags & (VRES_SELECT | VRES_REDO)))
		return vres_prio_check(req);
	else
		return 0;
}
#endif
...
int vres_shm_check_owner(vres_req_t *req, int flags)
{
	...
	if (vres_pg_own(page)) {
		...
		if (owner) {
			...
			if (ret) {
				vres_log_err(resource, "failed to change owner");
				goto out;
			}
		}
	}
	...
	if (vres_shm_need_forward(page, req)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to send to owner");
			goto out;
		}
	}
...
int vres_shm_propose(vres_req_t *req)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to get arg");
		return ret;
	}
...
int vres_shm_check_ttl(vres_req_t *req)
{
	...
	arg->ttl += 1;
	if (arg->ttl > VRES_SHM_TTL_MAX) {
		vres_log_err(resource, "op=%s, TTL is out of range", log_get_shmop(arg->cmd));
		return -EINVAL;
	}
...
vres_reply_t *vres_shm_fault(vres_req_t *req, int flags)
{
	...
	if (ret) {
		if ((flags & VRES_REDO) && (-EAGAIN == ret))
			return vres_reply_err(ret);
		else {
			vres_shm_handle_err(resource, ret);
			vres_log_err(resource, "op=%s, failed", log_get_shmop(arg->cmd));
		}
	}
...
int vres_shm_rmid(vres_req_t *req)
{
	...
	if (vres_is_owner(resource)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to notify members");
			return ret;
		}
	}
...
int vres_shm_passthrough(vres_t *resource, vres_page_t *page)
{
	int flags = vres_get_flags(resource);
	vres_id_t src = vres_get_id(resource);
...
vres_req_t *vres_shm_get_req(vres_t *resource)
{
	...
	if (vres_prio_check_live_time(resource, &arg->prio, &arg->live)) {
		vres_log_err(resource, "failed to get live time");
		free(req);
		return NULL;
	}
#endif
...
int vres_shm_check_arg(vres_arg_t *arg)
{
	...
	ret = vres_shm_passthrough(resource, page);
	if (ret)
		goto out;
	...
	if (vres_shm_need_priority(resource, page)) {
		...
		if (!req) {
			vres_log_err(resource, "failed to get request");
			ret = -EFAULT;
			goto out;
		}
		ret = vres_prio_check(req);
...
int vres_shm_get_arg(vres_t *resource, vres_arg_t *arg)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to get peers");
		goto out;
	}
	...
	if (!vres_pg_own(page)) {
		...
		if (ret) {
			vres_log_err(resource, "failed to get priority");
			goto out;
		}
	}
#endif
