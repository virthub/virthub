...
int vres_restore_resource(vres_t *resource, int flags)
{
	...
	while (pos < length) {
		...
		len = strlen(path);
		if (!len) {
			vres_log_err(resource, "failed to read path");
			ret = -EIO;
			break;
		}

		pos += len + 1;
		ret = tmp_dirname(path, dirname);
		if (ret) {
			vres_log_err(resource, "failed to get directory");
			break;
		}

		if (!tmp_is_dir(dirname)) {
			ret = tmp_mkdir(dirname);
			if (ret) {
		...
		if (!filp) {
			vres_log_err(resource, "failed to open file");
			ret = -ENOENT;
			if (buf)
				free(buf);
			break;
		...
		if (buf) {
			if (tmp_write(buf, 1, len, filp) != len) {
...
int vres_restore_task(vres_t *resource, int flags)
{
	...
	if (ret || (id != resource->owner)) {
		vres_log_err(resource, "failed to get task");
		return -EINVAL;
	}
...
int vres_restore(vres_t *resource, int flags)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to restore resource");
		goto out;
	}

	ret = vres_restore_task(resource, flags);
	if (ret) {
		vres_log_err(resource, "failed to restore task");
		goto out;
	}
