...
int vres_sem_create(vres_t *resource)
{
	...
	if (tmp_write((char *)sma, 1, size, filp) != size) {
...
int vres_sem_is_owner(vres_t *resource)
{
	char path[VRES_PATH_MAX];
...
vres_semundo_t *vres_sem_check_undo(vres_t *resource)
{
	...
	undos = tmp_get_desc(entry, vres_semundo_t);
	undo_size = vres_semundo_size(undos);
	total = tmp_items_count(entry, undo_size);
	for (i = 0; i < total; i++) {
...
int vres_sem_is_empy_undo(vres_semundo_t *un)
{
	if (un->pid < 0)
		return 1;
	else
...
int vres_sem_update_undo(vres_t *resource, vres_semundo_t *un)
{
	...
	if (entry) {
		int i;
		int total;
		vres_semundo_t *undos = tmp_get_desc(entry, vres_semundo_t);
	...
	if (!updated) {
		...
		ret = tmp_append(entry, (char *)un, undo_size);
	}
	tmp_put_entry(entry);
 	return ret;
}
