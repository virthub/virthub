...
static inline int vres_dump_file(char *path, void *arg)
{
	...
	buf = tmp_get_desc(entry, char);
	len = tmp_items_count(entry, 1);
	fprintf(filp, "%s\n", path);
...
int vres_dump_resource(vres_t *resource, int flags)
{
	...
	ret = tmp_handle_file(vres_dump_file, root, (void *)filp);
...
int vres_dump(vres_t *resource, int flags)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to check");
		goto out;
	}

	ret = vres_ckpt_clear(resource);
	if (ret) {
		vres_log_err(resource, "failed to clear");
		goto out;
	}
	...
	if (ret) {
		vres_log_err(resource, "failed to dump resource");
		goto out;
	}

	ret = vres_dump_task(resource, flags);
	if (ret) {
		vres_log_err(resource, "failed to dump task");
		goto out;
	}
