...
int vres_sync_request(vres_t *resource, vres_time_t *time)
{
	...
	if (ret || result.retval) {
		vres_log_err(&res, "failed to request");
...
vres_reply_t *vres_join(vres_req_t *req, int flags)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to lock");
		goto err;
	}
	...
	if (total < 0) {
		vres_log_err(resource, "failed to add member");
		ret = -EINVAL;
		goto out;
	}
	...
	if (public) {
		...
		if (ret) {
			vres_log_err(resource, "failed to get members");
			free(reply);
		}
	}
	log_sync(resource);
...
vres_reply_t *vres_leave(vres_req_t *req, int flags)
{
	...
	if (ret)
		vres_log_err(resource, "failed to destroy");
