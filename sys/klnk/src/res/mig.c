...
int vres_mig_check(vres_t *resource)
{
	...
	if (!tmp_access(path, F_OK)) {
		...
		if (tmp_access(path, F_OK)) {
			tmp_file_t *filp = tmp_open(path, "w");
			...
			if (tmp_write(&node_addr, sizeof(vres_addr_t), 1, filp) != 1) {
...
int vres_mig_set(vres_t *resource, vres_addr_t *addr, vres_arg_t *arg)
{
	...
	if (!filp) {
		vres_log_err(resource, "failed to open");
		ret = -ENOENT;
	} else {
		if (tmp_write(addr, sizeof(vres_addr_t), 1, filp) != 1) {
			vres_log_err(resource, "failed to write");
			ret = -EIO;
		}
		tmp_close(filp);
...
int vres_mig_loc(vres_t *resource, vres_addr_t *addr)
{
	...
	if (tmp_read(addr, sizeof(vres_addr_t), 1, filp) != 1) {
...
int vres_migrate(vres_t *resource, vres_arg_t *arg)
{
	...
	if (!path) {
		vres_log_err(resource, "failed to get path");
		return -EINVAL;
	}
	...
	if (total <= 0) {
		vres_log_err(resource, "failed to get nodes");
		return -EINVAL;
	}
	...
	if (!ret) {
		if (!memcmp(&addr, &node_addr, sizeof(vres_addr_t)))
			...
			if (vres_mig_set(resource, &addr, arg)) {
				vres_log_err(resource, "failed to set");
				ret = -EINVAL;
			}
		}
	} else {
