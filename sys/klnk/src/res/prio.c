...
int vres_prio_create(vres_t *resource, vres_time_t off)
{
	...
	tmp_entry_t *entry = tmp_get_entry(path, sizeof(vres_prio_t), TMP_RDWR | TMP_CREAT);
	if (!entry) {
		vres_log_err(resource, "no entry");
		return -ENOENT;
	}

	prio = tmp_get_desc(entry, vres_prio_t);
...
static int vres_prio_get_time(vres_t *resource, vres_time_t *time)
{
	...
	vres_get_priority_path(resource, path);
	entry = tmp_get_entry(path, sizeof(vres_prio_t), TMP_RDONLY);
	if (!entry) {
		vres_log_err(resource, "no entry");
		return -ENOENT;
	}
...
static int vres_prio_get_waittime(vres_t *resource, vres_time_t *waittime)
{
	...
	if (vres_prio_get_time(resource, &time)) {
		vres_log_err(resource, "failed to get time");
		return -EINVAL;
	}
...
static int vres_prio_convert_from_time(vres_t *resource, vres_id_t id, vres_time_t time)
{
	...
	if (pos < 0) {
		vres_log_err(resource, "invalid id");
		return -EINVAL;
	}
...
int vres_prio_get(vres_t *resource, vres_prio_t *prio)
{
	...
	ptr = tmp_get_desc(entry, vres_prio_t);
...
int vres_prio_set(vres_t *resource, vres_prio_t *prio)
{
	...
	vres_get_priority_path(resource, path);
	entry = tmp_get_entry(path, sizeof(vres_prio_t), TMP_RDWR);
	if (!entry) {
		vres_log_err(resource, "no entry");
		return -ENOENT;
	}
...
retry:
		...
		if (!vres_prio_is_expired(&prio)) {
			...
			if (ret) {
				vres_log_err(resource, "failed to get wait time");
				goto unlock;
			}
			vres_prio_unlock(resource);
			vres_prio_wait(time);
		...
		do {
			...
			if (ret) {
				vres_log_err(resource, "failed to get record");
				goto unlock;
			}
			id = vres_get_id(&record.req->resource);
			if ((id == user) || !user) {
...
int vres_prio_check(vres_req_t *req)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to extract priority");
		goto out;
	}
#endif
	...
	if (empty < 0) {
		vres_log_err(resource, "failed to check record");
		ret = -EINVAL;
		goto out;
	}
	...
	if (ret) {
		vres_log_err(resource, "failed to save record");
		goto out;
	}
	...
	if (empty) {
		...
		if (ret) {
			vres_log_err(resource, "failed to create thread");
			free(arg);
			goto out;
		}
	}
...
int vres_prio_sync_time(vres_t *resource)
{
	...
	if (start - prio.t_sync > VRES_PRIO_SYNC_INTERVAL) {
		...
		if (ret) {
			vres_log_err(resource, "failed to save priority");
			goto out;
		}
