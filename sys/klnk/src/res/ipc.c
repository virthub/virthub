...
int vres_ipc_check_arg(vres_arg_t *arg)
{
	...
	if (!ret || (-EAGAIN == ret)) {
		...
		vres_log_err(resource, "failed to get event");
		return ret;
	}
}
...
int vres_ipc_get(vres_t *resource, ipc_create_t create, ipc_init_t init)
{
	...
	if (ret) {
		vres_log_err(resource, "failed to check path");
	...
	if (ret) {
		vres_log_err(resource, "failed to create, ret=%d", ret);
		goto out;
	}

	ret = vres_member_create(resource);
	if (ret) {
		vres_log_err(resource, "failed to create member");
		goto out;
	}
	...
	if (owner) {
		if (create) {
			ret = create(resource);
			if (ret) {
				vres_log_err(resource, "failed to create owner");
				goto out;
			}
		}
		...
		if (ret) {
			vres_log_err(resource, "failed to join");
			goto out;
		}
		...
		if (result) {
			...
			free(result);
			if (ret) {
				vres_log_err(resource, "failed to save members");
				goto out;
			}
		}
	}
	...
	if (init) {
		ret = init(resource);
		if (ret) {
			vres_log_err(resource, "failed to initialize");
			goto out;
		}
	}
out:
...
int vres_ipc_put(vres_t *resource)
{
	...
	vres_rwlock_unlock(resource);
	if (ret) {
		vres_log_err(resource, "failed to leave");
		return ret;
	}
