...
int ckpt_dump_mem(char *node, struct task_struct *tsk, ckpt_desc_t desc)
{
	...
	if (ckpt_write(desc, &memory, sizeof(ckpt_mm_t)) != sizeof(ckpt_mm_t)) {
...
int ckpt_dump_signals(struct task_struct *tsk, ckpt_desc_t desc)
{
	...
	log_dump_signals("saving sigstack ...");
	...
	log_dump_signals("saving sigblocked ...");
	spin_lock_irq(&tsk->sighand->siglock);
	...
	log_dump_signals("saving pending ...");
	...
	log_dump_signals("saving shared_pending ...");
	...
	log_dump_signals("saving sigaction ...");
	for (i = 0; i < _NSIG; i++) {
		spin_lock_irq(&tsk->sighand->siglock);
...
int ckpt_dump_cred(struct task_struct *tsk, ckpt_desc_t desc)
{
	...
	if (cred.ngroups) {
		...
		if (ckpt_write(desc, groups, size) != size) {
			log_err("failed to dump group_info");
			ret = -EIO;
		}
		vfree(groups);
	}
...
int ckpt_dump_ext(struct task_struct *tsk, ckpt_desc_t desc)
{
	ckpt_ext_t ext;

	log_dump_ext("saving extra attributes ...");
	ext.personality = tsk->personality;
	ext.clear_child_tid = tsk->clear_child_tid;
	strncpy(ext.comm, tsk->comm, sizeof(ext.comm));

	if (ckpt_write(desc, &ext, sizeof(ckpt_ext_t)) != sizeof(ckpt_ext_t)) {
		log_err("failed to dump extra attributes");
		return -EIO;
	}
	log_dump_pos(desc);
	return 0;
