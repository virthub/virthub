...
int clone(char *node, int id)
{
	...
	if ('\0' != *node) {
		...
		sprintf(arg.path, "/vhub/mnt/%s/var/ckpt/%lx/tsk", node, (unsigned long)id);
	} else
		sprintf(arg.path, "/vhub/var/ckpt/%lx/tsk", (unsigned long)id);
...
int restore(char *node, int id)
{
	...
	if (0 == pid) {
		...
		ret = vres_request(node, VRES_CLS_TSK, id, VRES_OP_RESTORE, getpid(), 0);
		if (ret) {
			log_err("failed to send request");
			return ret;
		}
