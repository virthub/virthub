...
#define log_file_mkdir(path) log_debug("path=%s", path)
#else
#define log_file_mkdir(...) do {} while(0)
#endif

#ifdef LOG_FILE_RMDIR
#define log_file_rmdir(path) log_debug("path=%s", path)
#else
#define log_file_rmdir(...) do {} while(0)
#endif

#ifdef LOG_FILE_DIR_PUSH
#define log_file_dir_push(dir, file) log_debug("dir=%s, file=%s", dir, file)
#else
#define log_file_dir_push(...) do {} while(0)
#endif

#ifdef LOG_FILE_HANDLE_DIR
#define log_file_handle_dir(path) log_debug("path=%s", path)
#else
#define log_file_handle_dir(...) do {} while(0)
#endif

#ifdef LOG_FILE_HANDLE_FILE
#define log_file_handle_file(path) log_debug("path=%s", path)
#else
#define log_file_handle_file(...) do {} while(0)
#endif

#ifdef LOG_FILE_DELETE_FILE
#define log_file_delete_file(path) log_debug("path=%s", path)
#else
